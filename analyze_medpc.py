...
class MedPCAnalyzer:
    ...
    def load_and_process_data(self, bin_sizes=[5, 10, 30], selected_dates=None):
        """
        ...
        if selected_dates is not None:
            ...
            for date in selected_dates:
                date_df = self.summary_df.filter(pl.col('filename').str.contains(date))
                filtered_df.append(date_df)
            
            if filtered_df:
                self.summary_df = pl.concat(filtered_df)
        ...
        for bin_size in bin_sizes:
            ...
            # Filter by date if selected_dates is provided
            if selected_dates is not None:
                filtered_df = []
                for date in selected_dates:
                    date_df = df.filter(pl.col('session_id').str.contains(date))
                    filtered_df.append(date_df)
                
                if filtered_df:
                    df = pl.concat(filtered_df)
        ...
        if selected_dates is not None:
            filtered_df = []
            for date in selected_dates:
                date_df = self.time_series_df.filter(pl.col('session_id').str.contains(date))
                filtered_df.append(date_df)
            
            if filtered_df:
                self.time_series_df = pl.concat(filtered_df)
    ...
    def save_processed_data(self, selected_dates=None):
        ...
        # Get a parser instance with filtered data
    ...
    def plot_lever_presses_by_phase(self, bin_size=30, selected_dates=None):
        ...
        for i, phase in enumerate(phases):
            ...
            grouped = phase_data.group_by(['bin_start_min', 'bin_end_min'])
            ...
            mean_data = grouped.agg([
                ...
                pl.col('active_lever_presses').count().alias('active_count')
            ])
            ...
            # Calculate SEM manually using numpy
            active_stds = mean_data.select('active_std').to_series().to_list()
            inactive_stds = mean_data.select('inactive_std').to_series().to_list()
            counts = mean_data.select('active_count').to_series().to_list()
            
            active_sems = [std / np.sqrt(count) for std, count in zip(active_stds, counts)]
            inactive_sems = [std / np.sqrt(count) for std, count in zip(inactive_stds, counts)]
            
            bin_labels = [f"{start}-{end}" for start, end in zip(
                mean_data.select('bin_start_min').to_series().to_list(),
                mean_data.select('bin_end_min').to_series().to_list()
            )]
            ...
            if selected_dates:
                if len(selected_dates) == 1:
                    title += f" ({selected_dates[0]})"
                else:
                    title += f" ({len(selected_dates)} dates)"
            
            axes[i].set_title(title)
            axes[i].set_xlabel('Time (minutes)')
            axes[i].set_xticks(bin_centers)
            axes[i].set_xticklabels(bin_labels)
    ...
    def plot_time_series_data(self, selected_dates=None, selected_subjects=None):
        ...
        # Filter time series data by date if needed
        time_series_df = self.time_series_df
        if selected_dates:
            filtered_df = []
            for date in selected_dates:
                date_df = time_series_df.filter(pl.col('session_id').str.contains(date))
                filtered_df.append(date_df)
            
            if filtered_df:
                time_series_df = pl.concat(filtered_df)
        
        # Get unique subjects and phases
        subjects = time_series_df.select('subject').unique().to_series().to_list()
        
        # Filter by selected subjects if provided
        if selected_subjects:
            subjects = [s for s in subjects if s in selected_subjects]
        
        phases = time_series_df.select('phase').unique().to_series().to_list()
        
        for subject in subjects:
            # Filter for this subject
            subject_data = time_series_df.filter(pl.col('subject') == subject)
            ...
            for i, phase in enumerate(phases):
                ...
                if len(phase_data) == 0:
                    axes[i].set_title(f"Subject {subject}, {phase} - No data")
                    continue
                
                # Get session dates for this subject and phase
                session_ids = phase_data.select('session_id').unique().to_series().to_list()
                session_dates = [sid.split('_')[0] if '_' in sid else 'Unknown' for sid in session_ids]
                
                # Get dates string for title
                dates_str = ""
                if selected_dates:
                    if len(selected_dates) == 1:
                        dates_str = f" ({selected_dates[0]})"
                    else:
                        dates_str = f" ({', '.join(selected_dates)})"
                elif session_dates:
                    if len(session_dates) == 1:
                        dates_str = f" ({session_dates[0]})"
                    else:
                        dates_str = f" ({len(session_dates)} sessions)"
                ...
                if len(reinforced_data) > 0:
                    ...
                    axes[i].plot(reinforced_times, 
                               np.ones(len(reinforced_times)) * 0.6, 
                               'v', markersize=8, color='green', label='Reinforcers')
                
                # Customize plot
                axes[i].set_title(f"Subject {subject}, {phase}{dates_str}")
                axes[i].set_yticks([0.6, 0.8, 1.0])
                axes[i].set_yticklabels(['Reinforcers', 'Inactive', 'Active'])
            ...
            for i, phase in enumerate(phases):
                ...
                for resp_type, style in response_types.items():
                    ...
                    if len(resp_data) > 0:
                        ...
                        # Plot
                        axes[i].step(times, cumulative, 
                                   style['style'], color=style['color'], 
                                   label=style['label'], where='post')
                
                # Customize plot
                axes[i].set_title(f"Subject {subject}, {phase}{dates_str} - Cumulative Responses")
                ...
                axes[i].set_ylabel('Cumulative Responses')
    ...
    def plot_summary_by_phase(self, selected_dates=None):
        ...
        for i, (metric, label) in enumerate(metrics):
            ...
            for phase in phases:
                ...
                values = self.summary_df.filter(pl.col('phase') == phase).select(metric).to_series().to_list()
                
                # Add to lists
                all_values.extend(values)
                all_phases.extend([phase] * len(values))
            
            # Create box plot
            axes[i].boxplot([
                [v for v, p in zip(all_values, all_phases) if p == phase]
                for phase in phases if phase is not None
            ], labels=[p for p in phases if p is not None])
            
            # Add individual data points (jittered)
            for j, phase in enumerate([p for p in phases if p is not None]):
                phase_values = [v for v, p in zip(all_values, all_phases) if p == phase]
                
                # Add jitter
                x = np.random.normal(j + 1, 0.08, size=len(phase_values))
                
                axes[i].scatter(x, phase_values, alpha=0.5, color='black', s=20)
    ...
    def compare_active_vs_inactive(self, selected_dates=None):
        ...
        summary_df = self.summary_df.with_columns([
            (pl.col('active_lever_presses') / pl.when(pl.col('inactive_lever_presses') == 0).then(1).otherwise(pl.col('inactive_lever_presses'))).alias('active_inactive_ratio')
        ])
        ...
        for phase in phases:
            ...
            ratios = summary_df.filter(pl.col('phase') == phase).select('active_inactive_ratio').to_series().to_list()
            
            # Add to lists
            all_ratios.extend(ratios)
            all_phases.extend([phase] * len(ratios))
        
        # Create box plot
        ax.boxplot([
            [r for r, p in zip(all_ratios, all_phases) if p == phase]
            for phase in phases
        ], labels=phases)
        
        # Add individual data points (jittered)
        for i, phase in enumerate(phases):
            phase_ratios = [r for r, p in zip(all_ratios, all_phases) if p == phase]
            
            # Add jitter
            x = np.random.normal(i + 1, 0.08, size=len(phase_ratios))
            
            ax.scatter(x, phase_ratios, alpha=0.5, color='black', s=20)
    ...
    def run_analysis(self, bin_sizes=[5, 10, 30], selected_dates=None, selected_subjects=None):
        ...
        self.load_and_process_data(bin_sizes, selected_dates)
...
# selected_dates = ['2025-04-14']  # First day
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description='Analyze MedPC data')
